...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('sheyingzuopin','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","fontWeight":"bold","height":"24px"}'>{{sheyingzuopinCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>摄影作品总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)  ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('huodongbaoming','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"2"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#374254","fontWeight":"bold","height":"24px"}'>{{huodongbaomingCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>活动报名总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)        ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('zuopinchushou','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"1"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":" #374254","fontWeight":"bold","height":"24px"}'>{{zuopinchushouCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>作品出售总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type4" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="sheyingzuopinChart1" class="echarts1" v-if="isAuth('sheyingzuopin','首页统计')"></div>
		<div id="sheyingzuopinChart2" class="echarts2" v-if="isAuth('sheyingzuopin','首页统计')"></div>
		<div id="huodongbaomingChart1" class="echarts3" v-if="isAuth('huodongbaoming','首页统计')"></div>
		<div id="zuopinchushouChart1" class="echarts4" v-if="isAuth('zuopinchushou','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            sheyingzuopinCount: 0,
            huodongbaomingCount: 0,
            zuopinchushouCount: 0,
	...
	mounted(){
		this.init();
		this.getsheyingzuopinCount();
		this.sheyingzuopinChat1();
		this.sheyingzuopinChat2();
		this.gethuodongbaomingCount();
		this.huodongbaomingChat1();
		this.getzuopinchushouCount();
		this.zuopinchushouChat1();
	},
	methods:{
		// 词云
		...
		getsheyingzuopinCount() {
			this.$http({
				url: `sheyingzuopin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.sheyingzuopinCount = data.data
				}
			})
		},
		sheyingzuopinChat1() {
        ...
        var sheyingzuopinChart1 = echarts.init(document.getElementById("sheyingzuopinChart1"),'macarons');
        this.$http({
            url: "sheyingzuopin/group/zuopinfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].zuopinfenlei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].zuopinfenlei
                    })
                }
                var option = {};
				let titleObj = this.pie.title
				titleObj.text = '摄影作品统计'
                ...
                // 使用刚指定的配置项和数据显示图表。
                sheyingzuopinChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    sheyingzuopinChart1.resize();
                };
            }
        });
      })
    ...
    sheyingzuopinChat2() {
      this.$nextTick(()=>{
        ...
        var sheyingzuopinChart2 = echarts.init(document.getElementById("sheyingzuopinChart2"),'macarons');
        this.$http({
            url: "sheyingzuopin/group/yonghuxingming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                sheyingzuopinChart2.setOption(option);
				
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, sheyingzuopinChart2)
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    sheyingzuopinChart2.resize();
                };
            }
        });
      })
		...
		getzuopinchushouCount() {
			this.$http({
				url: `zuopinchushou/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.zuopinchushouCount = data.data
				}
			})
		},
		zuopinchushouChat1() {
        ...
        var zuopinchushouChart1 = echarts.init(document.getElementById("zuopinchushouChart1"),'macarons');
        this.$http({
            url: "zuopinchushou/group/zuopinfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                zuopinchushouChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zuopinchushouChart1.resize();
                };
            }
        });
      })
